...
int CSGD_FModManager::LoadSound(const char *szFilename, FMOD_MODE unMode )
{
	...
	int nHandle = -1;	//	an integer that is returned to the user so they can reference this sound again
...
bool CSGD_FModManager::UnloadSound( int nID )
{
	...
	if( m_SoundList[nID].ref == 0 || ( result = m_SoundList[nID].fmSound->release() ) == FMOD_OK )
	{
		...
		ZeroMemory(&m_SoundList[nID].unSoundFormat, sizeof(m_SoundList[nID].unSoundFormat));
		m_SoundList[nID].fmSound = NULL;
		m_SoundList[nID].ref = 0;
		m_SoundList[nID].fVolume = 1.0f;
		m_SoundList[nID].fPan = 0.0f;
		//m_SoundList[nID].fFreq = 44100.0f;
		bOutcome = true;
	}
	else
		FMODERR( result );
...
bool CSGD_FModManager::SetVolume( int nID, float fVolume )
{
	...
	//  set the volume of this sound object
	m_SoundList[nID].fVolume = fVolume;
...
bool CSGD_FModManager::SetFrequency( int nID, float fFreq )
{
	...
	//  set the frequency of this sound object
	//m_SoundList[nID].fFreq = fFreq;
...
bool CSGD_FModManager::SetPan( int nID, float fPan )
{
	...
	//  set the pan of this sound object
	m_SoundList[nID].fPan = fPan;
...
float CSGD_FModManager::GetVolume(int nID)
{
	assert( m_pSystem );
...
float CSGD_FModManager::GetFrequency(int nID)
{
	assert( m_pSystem );
	...
	return m_SoundList[nID].fFreq;
}


float CSGD_FModManager::GetPan(int nID)
{
	assert( m_pSystem );
